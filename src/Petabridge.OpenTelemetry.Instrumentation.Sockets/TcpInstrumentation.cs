...
public static class TcpInstrumentation
{
    ...
    /// <param name="onlyCareAboutTheseEndpoints">
    ///     Optional. When populated, we will only report
    ///     about activity involving these endpoints.
    /// </param>
    /// <remarks>
    ...
    /// </remarks>
    public static ISocketTelemetryConfigurator AddTcpConnectionInstrumentation(
        this ISocketTelemetryConfigurator builder,
        IPEndPoint[]? onlyCareAboutTheseEndpoints = null)
    {
        ...
        bool KeepListenerData(IPEndPoint endpoint)
        {
            ...
            foreach (var caredAboutEndpoint in onlyCareAboutTheseEndpoints)
                if (caredAboutEndpoint.Equals(endpoint))
                    return true;
        ...
        bool KeepConnectionData(TcpConnectionInformation connection)
        {
            ...
            foreach (var endpoint in onlyCareAboutTheseEndpoints)
                if (connection.LocalEndPoint.Equals(endpoint) || connection.RemoteEndPoint.Equals(endpoint))
                    return true;
